...
import type { GardenProject, GardenElement } from "@mity-garden/domain";
...
export interface ReferenceRenderOptions {
  ...
  cameraPosition?: { x: number; y: number; targetX: number; targetY: number };
...
function getElementColors(type: string): { fill: string; stroke: string } {
...
function drawNorthArrow(layer: Konva.Layer, stageW: number): void {
  const cx = stageW - 30;
  const cy = 30;
...
function drawScaleBar(layer: Konva.Layer, ppm: number, stageH: number): void {
