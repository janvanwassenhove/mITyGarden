...
type GmMapConstructor = new (el: HTMLElement, opts: {
    center: {
        lat: number;
        lng: number;
    };
    ...
    interface Window {
        google?: {
            maps: {
                ...
                event: {
                    addListener(target: GmDrawingManager, event: "polygoncomplete", handler: (polygon: GmPolygon) => void): unknown;
                };
                drawing: {
...
export declare function ProjectWizard({ onComplete, onCancel, mapsAdapter, googleMapsApiKey }: ProjectWizardProps): React.ReactElement;
export {};
